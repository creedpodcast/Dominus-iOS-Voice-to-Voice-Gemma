...
A third variant exists implicitly: the iOS build running on Apple Silicon Macs in Apple's "Designed for iPad" compatibility mode, which is what TestFlight and the App Store install on a Mac. `SUPPORTS_MAC_DESIGNED_FOR_IPHONE_IPAD = YES` exposes it as the "My Mac (Designed for iPad)" run destination in Xcode so it can be tested locally. This variant cannot render premium ("maui") system voices — the OS silently substitutes a low-quality voice instead of failing — so `SpeechManager` filters premium voices out of selection and the in-app picker there, blocklists any voice that renders zero audio and retries the utterance once with the next usable voice, and falls back gracefully. The app default voice is Daniel (en-GB), which ships preinstalled on both iOS and macOS and renders natively everywhere.
...
- Microphone access is requested explicitly on every platform. `WhisperManager` checks `AVAudioApplication.shared.recordPermission`, prompts when undetermined, retries once granted, and surfaces a user-visible error if denied. It also guards against the input format reporting `0 Hz / 0 channels` before a device/permission is bound, which would otherwise crash Core Audio on tap install. Mic and TTS pipeline failures set `micErrorMessage` / `audioErrorMessage`, which `ContentView` renders as a warning status pill instead of failing silently.
